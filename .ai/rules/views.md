...
## The top bar is FIRST in .shell's DOM, and that is load-bearing
DOM order inside `.shell` is **top bar -> banner -> rail -> canvas -> dock**, matching what is on screen; the grid places every region by `grid-area`, so the order is free to say what it means. The bar was LAST for a while, on the reasoning that Tab should reach the document before the chrome. That stopped being right once both panels defaulted to COLLAPSED on the editor: a collapsed panel is `display: none` and out of the tab order, so the toggles in the top bar are the ONLY way to open either one — and last in the DOM they were a keyboard reader's first visible control and their LAST tab stop, after the entire document. The skip link is what puts the document first for anyone who wants it there.

It is also what makes the editor's floating toolbar reachable by Tab at all. `resources/js/editor/ui/bubble.js` appends that toolbar to `<body>`, which is after `.shell`; with the bar last, tabbing out of the paper landed on the bar's rail toggle, the editor blurred, and the toolbar took itself out of the tab order before focus could ever come round to it. It cannot move INTO the canvas instead: `.canvas-region` carries `container-type: inline-size`, and a container-type applies `contain: layout`, which makes that element the containing block for every absolutely AND fixed positioned descendant — so a toolbar positioned from viewport rects would land offset by the rail's width and the bar's height, and the <900px `position: fixed` sheet would measure the canvas rather than the screen. `FairCopyTest::test_the_top_bar_leads_the_shell_in_dom_order` pins the order on three routes.
...
## A sheet renders INSIDE the page behind it, so its landmarks need their own names
A sheet is not a new document: `resources/views/livewire/documents/location-picker.blade.php` renders inside `documents/index.blade.php`, which already carries `<nav aria-label="Folder path">`. Two `<nav>` landmarks with the SAME name are two rows a screen reader cannot tell apart in its landmark list, so the picker's is `aria-label="Destination folder path"` — pinned from both sides by `SmartSaveTest::test_the_sheet_embeds_the_location_picker_and_names_the_destination`, which asserts both labels are present. Any landmark (`nav`, `aside`, `form`, `section[aria-label]`) added to a sheet, panel or partial gets a name that says WHICH one it is, not what kind of thing it is.
