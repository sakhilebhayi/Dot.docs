...
The fallback is right for an id that arrives from a BOOKMARK and wrong for one that arrives from a sheet somebody just filled in, so the smart-save path checks both: `LocationPicker::resolve()` authorises whatever node it is about to hand back through `ObjPolicy` (its `selectedId` is a public Livewire property and can be set without ever calling `selectFolder()`), and `Documents\Index::chosenLocation()` refuses outright when the picker's fallback LANDED somewhere other than the id the sheet asked for. `resolve(): Obj` is the one method callers use - Phase 3 extends what the picker OFFERS (recents, favourites, starred) without touching a single call site.
