...
## Every tree page shares BrowsesTheTree
App\Livewire\Documents\Index, App\Livewire\Files\Navigator and App\Livewire\Documents\LocationPicker (the smart-save sheet's location field) all stand in the tree, and the first two had their own copies of three small pieces: the currentTeam-then-personalTeam workspace lookup, the "resolve this folder id, fall back to my own root when it is missing, foreign or not a folder" rule, and guarded(), which turns a FilesService ValidationException into an error on the field that caused it. They live in App\Livewire\Files\BrowsesTheTree now. Put a new tree page on the trait rather than copying them again, and change the fallback rule in one place - it is a security-shaped rule (a foreign id must show YOUR root, never theirs, and never a 403 that confirms the folder exists).

The fallback is for an id that arrives from a BOOKMARK. An ACTION TARGET never goes through it: it is found, authorised through `ObjPolicy` and checked for being a folder, directly, the way `Files\Navigator::node()` does it. `Documents\Index::chosenLocation()` follows that rule exactly - there is NO special case here. Inferring a refusal by watching whether the fallback substituted (which is what it used to do) is both indirect and wrong: it cannot tell a folder in another team (403) from a folder DELETED while the sheet was open (404 with a message the writer can read, since their name and template are still in the sheet). `LocationPicker::resolve()` authorises its own answer through `ObjPolicy` as well - its `selectedId` is a public Livewire property and can be set without ever calling `selectFolder()`, so the method cannot lean on the component's happy path (`LocationPickerTest::test_resolve_refuses_a_folder_outside_the_users_team_on_its_own` pins that gate on its own).

**The two gates are separate, and `resolve()` has no production caller.** `Index::createDocumentAtLocation()` does not ask the embedded picker for the node: it re-derives it from its own `$newDocumentFolderId` through `chosenLocation()`, which runs the find + authorise + is-it-a-folder itself (the picker is a child component, and the parent must not take a child's word for an id it will file a document at). `resolve()` is the picker's own gate on the same question, covered by its own tests, and the sheet's markup is the only thing wired to the picker today. Phase 3 has to touch BOTH to make recents/favourites/starred reach document creation - extending `LocationPicker` alone changes what the sheet OFFERS and nothing about what `chosenLocation()` will accept.

A document's `team_id` is the LOCATION's team, never the session's `currentTeam`. `DocumentStore::create()` defaults it to `$owner->currentTeam`, while `FilesService::registerDocument()` stamps the tree node with `$parent->team_id`; a document filed into another of the writer's teams then has a node and a row that disagree, and team B lists a title in the Navigator (`children()` runs no `DocumentPolicy`) that nobody there can open. Every caller of `DocumentStore::create()` that passes a `$parent` passes `'team_id' => $parent->team_id` with it - `Navigator::createDocument()`, `Navigator::importHere()` and `Documents\Index::createDocumentAtLocation()`.
