...
**Architecture:** A debounced measurement pass walks the top-level blocks of `.paper` in the live DOM, a pure algorithm (`measure.js`) decides where pages break (block-level only — headings/figures/tables/callouts never orphaned; tables and lists split between rows/items, with a table's header row height reserved on every continuation (the header's own DOM is not yet visually cloned there — a named v1 deferral, design spec §7); a single stranded paragraph line is possible in v1), and a ProseMirror plugin renders the result as non-editable **widget decorations** (gap, shadow, header/footer bands) inserted at computed positions — never written into the document JSON. Header/footer template substitution is extracted from `PrintRenderer` into a shared `HeaderFooterBands` class so the live view and the PDF export use the exact same field-injection-safe logic.

**Post-implementation note (whole-branch review, final fix wave):** this plan document reflects the ORIGINAL 8-task design, written and refined DURING implementation as real bugs were found (see the SDD ledger for the full history). Two things changed after this document was last edited and are NOT reflected in its task bodies below, but ARE reflected in the actual shipped code and the design spec's final version: (1) **Two Page mode was removed from v1** (Task 8) — the code has five view modes, not six, `two-page` no longer appears in `MODES`/the `<select>`/`CssBuilder`; (2) the **measurement pass excludes its own leftover widget decorations** when re-measuring a paragraph or list a previous pass already split (whole-branch review finding) — `decorations.js`'s `measureLines()`/`measureBlocks()` filter out `.dotdoc-page-boundary`/`.dotdoc-page-edge` elements rather than including their height/line-rects in a block's measurement. Treat the design spec and the actual code, not this plan's task bodies, as authoritative for both.
