...
**Known gap, found in Task 8's browser verification (§7):** `measure.js`'s break-position math correctly *reserves* the header row's height on every continuation page (so the break-position/page-count math is right), but nothing yet *clones the header row's DOM* into the continuation — v1 ships with the header visually appearing once, at the top of the table, not repeated at each split. See §7.
...
| **Focus** | Hides page-break decorations, headers/footers, and all chrome (rail/dock/topbar) — a single continuous scroll of prose, closest to the pre-pagination canvas. **v1 gap (§7):** only the page-break decorations/headers/footers are hidden; the rail/dock/topbar chrome-hiding half is not yet implemented — it needs to interact with the rail/dock panel-state machine (a panel the writer opened on purpose should very likely reappear as it was on the way out of Focus mode), a real decision rather than a CSS one-liner. |
...
- Browser verification: a long document produces the expected page count and visible breaks; a manual page break forces a boundary regardless of remaining space; a table splits between rows with space correctly reserved for the header row (visual repetition of the header itself is a named v1 gap, §7); switching each of the five shipped view modes renders correctly in both night and day (Two Page deferred, §7; Focus mode's chrome-hiding half also deferred, §7) — confirmed live in Task 8's browser verification, along with a dark-mode thumbnail legibility bug found by that same pass and fixed before this plan's whole-branch review; Impeccable and the rendered-pair contrast script are re-run on the editor page with pagination active, matching the Fair Copy bar.
...
- **A split table's header row is not visually cloned onto the continuation** (found live in Task 8's browser verification) — `measure.js`'s break-position math already reserves the header's height on every continuation page, so page counts and break positions are correct, but nothing in `pagination/decorations.js` yet clones the header `<tr>` into a small `<table>` of its own at each split point. Doing so needs live column widths matched to the original table and the cloned header excluded from the thumbnail Range-extraction — a real, separable piece of work once block-level pagination (this phase) is proven, not a verification-sweep-sized fix.
- **Focus mode's chrome-hiding is only half-built** (found live in Task 8's browser verification) — page-break decorations, headers and footers are hidden, but the rail/dock/topbar chrome named in §3 is not. Doing this correctly needs a decision about the existing rail/dock panel-state machine (a panel the writer opened on purpose very likely should reappear as it was on the way out of Focus mode, not just be force-closed), not a one-line CSS rule.
