<div class="page page-narrow">
    <div class="page-head">
        <div>
            ...
            <p class="page-lede">
                ...
                Write <code class="micro">{content}</code> where the document's text should go.
            </p>
        </div>
        ...
        <section class="panel" aria-labelledby="cmd-form">
            ...
            <div class="panel-body">
                <div class="grid-2">
                    <div class="field-row" style="margin-top:0">
                        ...
                        <div class="toolbar" style="gap:0">
                            <span class="micro" style="padding:8px var(--s2);border:1px solid var(--line);border-right:0">/</span>
                ...
                <div class="field-row">
                    ...
                    <p class="field-hint">Use <code class="micro">{content}</code> to drop the document's text into the prompt.</p>
    ...
    <section class="panel" aria-labelledby="cmd-list">
        <div class="panel-head">
            ...
            <span class="micro">
                <x-shell.figure :value="$commands->count()" label="Commands" />
            </span>
        </div>
        ...
        @else
            <ul class="list">
                @foreach ($commands as $cmd)
                    <li class="list-row">
                        <span class="list-key">
                            <span class="micro">/{{ $cmd->name }}</span>
                            <span class="list-sub">
                                {{ $cmd->description ?: Str::limit($cmd->prompt_template, 90) }}
                            </span>
                        </span>
                        @if ($cmd->share_with_team)
                            <span class="list-val">Team</span>
                        @endif
                        @if ($cmd->user_id !== auth()->id())
                            <span class="list-val">Shared with you</span>
                        @else
